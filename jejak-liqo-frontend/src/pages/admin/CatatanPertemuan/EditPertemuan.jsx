...
const EditPertemuan = () => {
  ...
  const fetchMeetingData = async () => {
    try {
      ...
      // Set group name for display
      if (meeting.group) {
        setGroupName(`${meeting.group.name} - ${meeting.group.mentor?.name || 'No Mentor'}`);
      }

      // Load attendances or fetch mentees
      if (meeting.attendances && meeting.attendances.length > 0) {
        // Use existing attendances
        const existingAttendances = meeting.attendances.map(attendance => ({
          mentee_id: attendance.mentee.id,
          mentee_name: attendance.mentee.full_name,
          status: attendance.status,
          notes: attendance.notes || ''
        }));
        setAttendances(existingAttendances);
      } else if (meeting.group?.id) {
        // Fetch mentees and create default attendances
        await fetchAndCreateAttendances(meeting.group.id);
      }
    } catch (error) {
  ...
  const fetchAndCreateAttendances = async (groupId) => {
    try {
      const response = await meetingsAPI.getGroupMentees(groupId);
      const mentees = Array.isArray(response.data.data) ? response.data.data : [];
      
      const defaultAttendances = mentees.map(mentee => ({
        mentee_id: mentee.id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Gagal memperbarui pertemuan');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="p-3 sm:p-6">
        ...
        >
          ...
          <div>
            ...
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Perbarui informasi pertemuan dan kehadiran mentee
            </p>
          </div>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  value={groupName}
                  disabled
                  className={`w-full px-4 py-3 rounded-xl border transition-colors opacity-60 cursor-not-allowed ${
                    isDark 
          ...
          >
            ...
            >
              {loading ? 'Menyimpan...' : 'Perbarui Pertemuan'}
            </button>
          </motion.div>
        </form>
      </div>
