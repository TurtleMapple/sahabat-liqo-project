...
const ExistingMenteeSelector = ({ groupId, user, onBack, group }) => {
  ...
  const fetchAllMentees = async () => {
    try {
      ...
      const response = await getAllMentees({ per_page: 1000 }); // Get all mentees
      console.log('All mentees response:', response.data);
      console.log('Total mentees from API:', response.data?.length);
      
      // Get target gender from group data
      const targetGender = group?.mentor?.profile?.gender || user?.profile?.gender;
      console.log('Target gender:', targetGender);
      console.log('Group data:', group);
      console.log('User data:', user);
      
      // Debug: Check gender distribution
      const genderCounts = response.data.reduce((acc, mentee) => {
        acc[mentee.gender] = (acc[mentee.gender] || 0) + 1;
        return acc;
      }, {});
      console.log('Gender distribution:', genderCounts);
      
      // Filter mentees by gender - temporarily show all if no target gender
      let filteredMentees;
      if (targetGender) {
        filteredMentees = response.data.filter(mentee => 
          mentee.gender === targetGender
        );
        console.log(`Filtered mentees by gender ${targetGender}:`, filteredMentees.length);
      } else {
        filteredMentees = response.data;
        console.log('No target gender found, showing all mentees:', filteredMentees.length);
      }
  ...
  const filteredMentees = allMentees.filter(mentee => {
    ...
    // Exclude mentees already in current group
    const notInCurrentGroup = mentee.group_id !== parseInt(groupId);
    
    if (filterType === 'available') {
      // Hanya mentee yang belum memiliki kelompok
      const result = matchesSearch && mentee.group_id === null;
      return result;
    } else if (filterType === 'hasGroup') {
      // Semua mentee yang sudah ada di kelompok lain (bukan kelompok saat ini)
      const result = matchesSearch && mentee.group_id !== null && notInCurrentGroup;
      return result;
    }
  ...
  console.log('Filter results:', {
    ...
    filteredCount: filteredMentees.length,
    availableCount: allMentees.filter(m => m.group_id === null).length,
    hasGroupCount: allMentees.filter(m => m.group_id !== null && m.group_id !== parseInt(groupId)).length
  });
  ...
  return (
    <>
      <ConfirmModal
        ...
        groupName={group?.group_name || 'kelompok ini'}
      />
                ...
                <div className="flex space-x-6 mb-3">
                  <label className="flex items-center">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      Mentee Tersedia ({allMentees.filter(m => m.group_id === null).length})
                    </span>
                  </label>
                  <label className="flex items-center">
                    <input
                    ...
                    />
                    <span className="text-sm text-gray-700 dark:text-gray-300">
                      Mentee yang Sudah Ada Grup ({allMentees.filter(m => m.group_id !== null && m.group_id !== parseInt(groupId)).length})
                    </span>
                  </label>
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  Total mentee dengan gender yang sama: {allMentees.length} | 
                  Ditampilkan: {filteredMentees.length}
                </div>
              </div>
              ...
              {filteredMentees.length > 0 ? (
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500 dark:text-gray-400">
                                {mentee.nickname && `${mentee.nickname} • `}
                                {hasGroup ? `Kelompok: ${mentee.group?.group_name || 'Tidak diketahui'}` : 'Belum ada kelompok'}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center space-x-2">
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              ...
                              {hasGroup ? 'Ada Kelompok' : 'Tersedia'}
                            </span>
                            {isSelected && (
                ...
                <div className="text-center py-8 mb-6">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                    {searchQuery ? 'Mentee Tidak Ditemukan' : 
                     filterType === 'available' ? 'Tidak Ada Mentee Tersedia' : 'Tidak Ada Mentee di Kelompok Lain'}
                  </h3>
                  <p className="text-gray-500 dark:text-gray-400">
                    {searchQuery ? 'Coba kata kunci lain' : 
                     filterType === 'available' ? 'Semua mentee sudah memiliki kelompok' : 'Tidak ada mentee di kelompok lain dengan gender yang sama'}
                  </p>
                </div>
              )}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const menteesWithGender = mentees.map(mentee => ({
        ...mentee,
        gender: group?.gender || group?.mentor?.gender || user?.profile?.gender || 'Akhwat'
      }));
                  ...
                  <div key={index} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={group?.gender || group?.mentor?.gender || user?.profile?.gender || 'Akhwat'}
                          disabled
