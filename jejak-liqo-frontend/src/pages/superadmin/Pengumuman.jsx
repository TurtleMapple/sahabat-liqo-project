...
const transformAnnouncementData = (backendAnnouncement) => ({
  ...
  tanggal_dibuat: new Date(backendAnnouncement.event_at).toLocaleDateString('id-ID', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    timeZone: 'UTC'
  }),
  waktu_dibuat: new Date(backendAnnouncement.event_at).toLocaleTimeString('id-ID', {
    hour: '2-digit',
    minute: '2-digit',
    timeZone: 'UTC'
  })
...
const Pengumuman = () => {
  ...
  const fetchAnnouncements = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/announcements?per_page=${itemsPerPage}&page=${currentPage}&status=active`);
