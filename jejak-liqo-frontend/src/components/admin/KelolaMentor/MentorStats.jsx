...
const MentorStats = ({ stats }) => {
  ...
  const getGenderStats = () => {
    return {
      ikhwanData: { 
        total: stats.total_ikhwan || 0, 
        active: stats.active_ikhwan || 0
      },
      akhwatData: { 
        total: stats.total_akhwat || 0, 
        active: stats.active_akhwat || 0
      }
    };
  };

  const { ikhwanData, akhwatData } = getGenderStats();
        ...
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>
              {stats.total_mentors} / {stats.active_mentors}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>
              {ikhwanData.total} / {ikhwanData.active}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>
              {akhwatData.total} / {akhwatData.active}
            </p>
          </div>
        </div>
      </motion.div>
